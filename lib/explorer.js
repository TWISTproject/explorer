...
module.exports = {
  ...
  get_claims_for_name: function(name, cb)
  {
      var uri = base_url + 'getclaimsforname?name=' + name;
      request({uri: uri, json: true}, function (error, response, body) {
        return cb(null, body);
      });
  },

  get_claim_from_claim_id: function(name, claimId, cb)
  {
      var uri = base_url + 'getclaimsforname?name=' + name;
      request({uri: uri, json: true}, function (error, response, body) {
        for (var i = 0; i < body.claims.length; ++i) {
          if (body.claims[i].claimId == claimId) {
            cb(null, body.claims[i]);
              return;
          }
        }
        cb('no claim found for name "' + name + '" and claim id ' + claimId);
      });
  },

  get_claims_for_tx: function (tx, cb) {
    var uri = base_url + 'getclaimsfortx?txid=' + tx;
    request({uri: uri, json: true}, function (error, response, body) {
      if (error)
      {
        cb(error, {});
      ...
      cb(null, {claims: claims, supports: supports});
    });
  }
